  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="flex gap-1 mb-6 bg-[var(--bg-card)] rounded-xl p-1 border border-[var(--border-light)]">
        {tabs.map(tab => (
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-body font-medium transition-all duration-200 flex items-center justify-center gap-1.5
              ${activeTab === tab.id
      ...
      <div className="space-y-3">
        {current.map(friend => (
          <div key={friend.id} className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-light)] p-4 flex items-center gap-4 animate-fade-in">
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              {activeTab === "requests" && (
                <>
                  <button
                    ...
                    className="px-3 py-1.5 bg-[var(--text-primary)] text-white rounded-xl text-xs font-body font-medium hover:opacity-90 transition-opacity"
                  >
                    Принять
                  </button>
                  <button
              ...
              {activeTab === "suggestions" && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-[var(--border-light)] text-[var(--text-secondary)] rounded-xl text-xs font-body hover:bg-[var(--bg-hover)] transition-colors"
                >
